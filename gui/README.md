...
permet d'override le parametrage des packages :
on override niri par celui de niri-flake
on cree kernllvmPackages pour override le parametrage par defaut de llvmPackages
...
...
le mdp correspond au secret dans password.json gérer par sops-nix
...
| terminal |  foot  | moderne, minimaliste, légé, rapide et complet  [ranking](https://ucs-detect.readthedocs.io/results.html#general-tabulated-summary) |
...
Integre la configuration de plusieurs app (module foot, noctalia) et du theming (curseur, icon, theme...)
...
Ici le disque est compressé en ZSTD compression level 1. LZ4 aurait été plus performant (ce qu'on utilise pour le server sous ZFS) vu qu'on ne cherche pas non plus une forte compression mais n'est pas supporté
